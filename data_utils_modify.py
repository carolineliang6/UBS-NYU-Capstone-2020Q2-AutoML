...
def processCriteoAdData(d_path, d_file, npzfile, split, convertDicts, pre_comp_counts,sparse_count):
...
def concatCriteoAdData(
    ...
    if memory_map:
        ...
        den_fea = dense_count  # 13 dense  features
        spa_fea = sparse_count  # 26 sparse features
...
def transformCriteoAdData(X_cat, X_int, y, days, data_split, randomize, total_per_file):
    # Transforms Criteo Kaggle or terabyte data by applying transformation
...
def getCriteoAdData(
    ...
    def process_one_file(
        ...
        with open(str(datfile)) as f:
            ...
            X_int = np.zeros((num_data_in_split, dense_count), dtype="i4")  # 4 byte int
            X_cat = np.zeros((num_data_in_split, sparse_count), dtype="i4")  # 4 byte int
            ...
            for k, line in enumerate(f):
                ...
                if sub_sample_rate>1.0 and target:
                    label_1.append(i)
                
                if (sub_sample_rate < 1) and (target == 0) and \
                   (rand_u if sub_sample_rate == 0.0 else rand_u[k]) < sub_sample_rate:
                    continue

                y[i] = target
                X_int[i] = np.array(line[1:(dense_count+1)], dtype=np.int32)
                if max_ind_range > 0:
                    X_cat[i] = np.array(
                        list(map(lambda x: int(x, 16) % max_ind_range, line[(dense_count+1):])),
                        dtype=np.int32
                    )
                else:
                    X_cat[i] = np.array(
                        list(map(lambda x: int(x, 16), line[(dense_count+1):])),
                        dtype=np.int32
                    )
                # count uniques
                for j in range(sparse_count):
            ...
            if sub_sample_rate>1.0:
                sub_sample_index = np.random.choice(label_1, int((sub_sample_rate-1)*i), replace=True)
                for s_index in sub_sample_index:
                    y[i], X_int[i], X_cat[i]  = y[s_index], X_int[s_index], X_cat[s_index]
                    i += 1
    ...
    recreate_flag = False
    convertDicts = [{} for _ in range(sparse_count)]
    ...
    # dictionary files
    counts = np.zeros(sparse_count, dtype=np.int32)
    if recreate_flag:
        # create dictionaries
        for j in range(sparse_count):
    ...
    # process all splits
    processCriteoAdData(d_path, d_file, npzfile, days, convertDicts, counts,sparse_count)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dense-count", type=int, default="13")
    parser.add_argument("--sparse-count", type=int, default="26")
