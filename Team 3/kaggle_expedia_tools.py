...
def df_from_query(row_start = 7000014, row_end = 9917530, training = True, srch_start = None, srch_end = None):
    ...
    #train_dfpropmissing = sql.frame_query(main_cols + agg_missing + from_tables2 + " WHERE prop_id NOT IN (SELECT prop_id FROM " + missing_from_table + ")"
    # + where_str, con = dbcon,  =index_col 'row_num')
    #depreciation
    sql1 = main_cols + agg_cols + from_tables1 + " WHERE p.prop_id = s.prop_id" + where_str
    train_dfmost = pd.read_sql_query(sql1, con=dbcon, index_col='row_num')
    sql2 = main_cols + agg_missing + from_tables2 + " WHERE prop_id NOT IN (SELECT prop_id FROM " + missing_from_table +")"+ where_str
    train_dfpropmissing = pd.read_sql_query(sql2, con=dbcon, index_col='row_num')
    dbcon.close()
...
def test_pred_sorted(test_df, model, cols = None, regress_model = False):
    ...
    if regress_model:
        ...
        scores = model.predict_proba(test_df[cols])
        scores = scores[:, 1]
